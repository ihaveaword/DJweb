...
// AI 回复数据库（备用/离线模式）
const aiResponses = {
    '介绍一下你自己': '你好！我是董晶，华北电力大学英语语言文学专业的硕士研究生。',
    ...
    'default': '这是一个很好的问题！作为董晶的AI助手，我可以回答关于她的专业背景、证书和联系方式的问题。\n\n你可以问我：\n• 关于她的专业背景\n• 她获得的证书和荣誉\n• 如何联系她\n\n或者你可以点击上面的快速问题按钮来开始对话！'
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 初始化学术旅行语言切换
    initAcademicLanguageToggle();
    console.log('✅ 学术旅行语言切换功能已初始化');
...
function initAIChat() {
    ...
    function getAIResponse(userMessage) {
        ...
        if (userMessage.includes('你好') || userMessage.includes('嗨') || userMessage.includes('hi') || userMessage.includes('hello')) {
            return '你好！很高兴见到你！我可以帮你了解更多关于董晶的信息。有什么我可以帮助你的吗？';
        }
...
// 学术旅行语言切换
function initAcademicLanguageToggle() {
    const toggle = document.getElementById('academicLangToggle');
    if (!toggle) return;
    ...
    // 获取学术旅行区域的所有文本元素
    const academicSection = document.querySelector('.academic-timeline');
    ...
    toggle.addEventListener('click', (e) => {
        ...
        // 切换标题
        const zhTitle = document.querySelector('.academic-journey-header .section-title.text-zh');
        const enTitle = document.querySelector('.academic-journey-header .section-title.text-en');
        
        if (currentLang === 'zh') {
            if (zhTitle) zhTitle.style.display = 'block';
            if (enTitle) enTitle.style.display = 'none';
        } else {
            if (zhTitle) zhTitle.style.display = 'none';
            if (enTitle) enTitle.style.display = 'block';
        }

        // 切换时间线内容
        const zhMain = academicSection.querySelectorAll('.timeline-main.text-zh');
        const enMain = academicSection.querySelectorAll('.timeline-main.text-en');
        const zhDetails = academicSection.querySelectorAll('.timeline-details.text-zh');
        const enDetails = academicSection.querySelectorAll('.timeline-details.text-en');
        ...
        if (currentLang === 'zh') {
            ...
            zhDetails.forEach(el => {
                el.style.display = 'block';
            });
            enDetails.forEach(el => {
                el.style.display = 'none';
            });
        } else {
            zhMain.forEach(el => {
